...
- [x] A valid `join_room` receives `room_state`; all peers receive exactly one
  `user_joined` and one eventual `user_left`.
- [x] Duplicate usernames remain distinct and cannot modify each other's presence.
- [x] Pre-join messages, repeated joins, unsupported rooms, and mismatched room IDs
  return the specified errors without mutating room state.
- [x] The specification's error behavior is finalized for the implemented state
...
- [x] Connection state-machine tests pass.
- [x] Two-client WebSocket integration tests cover join, duplicate labels, snapshot,
  and disconnect.
- [x] `gleam format --check src test`
- [x] `gleam build`
- [x] `gleam test`
