...
pub fn start_server(
  ...
  TestServer(started.pid, directory, port)
}

pub fn stop_server(server: TestServer) -> Nil {
  let TestServer(root, _, _) = server
  let _ = stop_supervisor(root)
  Nil
}

pub fn server_port(server: TestServer) -> Int {
  let TestServer(_, _, port) = server
  port
}

pub fn server_directory(server: TestServer) -> room_directory.RoomDirectory {
  let TestServer(_, directory, _) = server
  directory
}

pub fn get_http(port: Int, path: String) -> HttpResponse {
...
pub fn websocket_status(port: Int, host: String, origin: String) -> Int {
  let socket = open_socket(port)
  let request = websocket_request(host, origin)
  let assert Ok(Nil) = tcp.send(socket, bytes_tree.from_string(request))
  let assert Ok(raw_response) = read_http_bytes(socket, <<>>)
  let _ = tcp.close(socket)
  let assert Ok(response) = decode_http_response(raw_response)
...
pub fn connect_websocket(port: Int, host: String, origin: String) -> Client {
  let socket = open_socket(port)
  let request = websocket_request(host, origin)
  let assert Ok(Nil) = tcp.send(socket, bytes_tree.from_string(request))
...
pub fn send_join(client: Client, username: String) -> Nil {
...
pub fn send_message(client: Client, text: String) -> Nil {
  let Client(socket, _) = client
  let payload =
...
pub fn decode_room_state(payload: String) -> Result(WireRoomState, Nil) {
  json.parse(payload, using: room_state_decoder())
  |> result_replace_error()
}

pub fn decode_user_joined(payload: String) -> Result(WireUserJoined, Nil) {
  json.parse(payload, using: user_joined_decoder())
  |> result_replace_error()
}

pub fn decode_user_left(payload: String) -> Result(WireUserLeft, Nil) {
  json.parse(payload, using: user_left_decoder())
  |> result_replace_error()
}

pub fn decode_message_sent(payload: String) -> Result(WireMessageEvent, Nil) {
  json.parse(payload, using: message_sent_decoder())
  |> result_replace_error()
}

pub fn decode_error(payload: String) -> Result(WireError, Nil) {
...
fn websocket_request(host: String, origin: String) -> String {
  "GET /ws HTTP/1.1\r\n"
  <> "Host: "
  <> host
  <> "\r\n"
