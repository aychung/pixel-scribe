...
pub type DecodeFailure {
  DecodeFailure(error: DecodeError, room_id: Option(domain.RoomId))
}
...
pub fn decode_client_event(
  ...
  decode_client_event_with_context(frame)
  |> result.map_error(fn(failure: DecodeFailure) { failure.error })
}

pub fn decode_client_event_with_context(
  frame: ClientFrame,
) -> Result(ClientEvent, DecodeFailure) {
  case frame {
    BinaryFrame(_) -> Error(DecodeFailure(BinaryPayload, None))
    TextFrame(payload) -> {
      let bytes = payload |> bit_array.from_string |> bit_array.byte_size

      case bytes > max_event_bytes {
        True -> Error(DecodeFailure(PayloadTooLarge, None))
        False ->
          case json.parse(from: payload, using: raw_client_event_decoder()) {
            Ok(event) -> validate_raw_event_with_context(event)
            Error(_) -> Error(DecodeFailure(MalformedEvent, None))
          }
      }
    }
  }
...
fn validate_raw_event_with_context(
  event: RawClientEvent,
) -> Result(ClientEvent, DecodeFailure) {
  case event {
    RawInvalid -> Error(DecodeFailure(MalformedEvent, None))
    RawJoinRoom(raw_room_id, raw_username) -> {
      case domain.new_room_id(raw_room_id) {
        Error(_) -> Error(DecodeFailure(InvalidRoomIdValue, None))
        Ok(room_id) ->
          case domain.new_username(raw_username) {
            Error(_) ->
              Error(DecodeFailure(InvalidUsernameValue, Some(room_id)))
      ...
      case domain.new_room_id(raw_room_id) {
        ...
        Ok(room_id) ->
          case domain.new_message_text(raw_text) {
            Error(_) ->
              Error(DecodeFailure(InvalidMessageTextValue, Some(room_id)))
