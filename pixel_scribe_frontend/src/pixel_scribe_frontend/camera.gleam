...
pub fn new(
  ...
  let viewport =
    ViewportExtent(
      width: normalize_extent(viewport_width),
      height: normalize_extent(viewport_height),
    )

  build(viewport, self_id, placements)
}
...
pub fn resize(
  ...
  let Camera(_, _, self_id) = camera
  new(viewport_width, viewport_height, self_id, placements)
}
...
pub fn retarget(
  ...
  let Camera(viewport, _, _) = camera
  build(viewport, self_id, placements)
}
...
pub fn update(
  ...
  let Camera(viewport, _, self_id) = camera
  build(viewport, self_id, placements)
}
